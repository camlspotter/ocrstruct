...
- google/gemma-4-26B-A4B-it (FP8) (thinking=false) 
- gpt-5
- gpt-5-mini
- gpt-5-nano
- gpt-5.2
...
`google/gemma-4-26B-A4B-it (thinking=false)` ではモデルロード時に FP8 に量子化を行う。
この量子化のため、立ち上げに 15分ほど必要となる。

OpenAI モデルについては `reasoning_effort` の指定を行っていない。
一部モデルでは既定値が `none` と明記されているが、全モデルで未指定時の挙動を断定はしない。

これらを組み合わせ、合計 106 x 12 = 1272 の結果を得た。
...
| `google/gemma-4-26B-A4B-it [thinking=false]` | 1.65s | 1.66s | $0.0 | $0.0 |
...
Open weight model では `Qwen/Qwen3.6-27B-FP8 [thinking=false]` と
`google/gemma-4-26B-A4B-it [thinking=false]` がほぼ同じ 0.865, 0.864 を取っている。

最速は `Qwen/Qwen3.6-35B-A3B-FP8 [thinking=false]` の 1.6s だった。
...
本 screening タスクでは `thinking=true` の採用意義はないと思われる。
...
`google/gemma-4-26B-A4B-it [thinking=false]`。0.864 の高得点で平均 1.65s と非常に早い。
`Qwen/Qwen3.6-27B-FP8 [thinking=false]`。の 0.865 はほんの少し上回るが、誤差の範囲。時間は平均 7.5s で `gemma-4-26B-A4B-it` には劣る。
