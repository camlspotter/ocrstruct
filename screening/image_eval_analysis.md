...
- `none`: 説明文や OCR を追加しても、RAG 上の価値はほとんど期待できない。これ以上の解析を行うべきではない。
...
- google/gemma-4-26B-A4B-it [thinking=false]
- gpt-5
- gpt-5-mini
- gpt-5-nano
- gpt-5.2
...
Qwen 3.6 系では `thinking=true` と同等。

Qwen と gemma については DGX Spark 互換機を利用し、そこでの処理時間を計測し、費用は0とした。

OpenAI モデルについては `reasoning_effort` の指定を行っていない。この場合、多くのモデルで thinking 設定は最低とドキュメントされている。

これらを組み合わせ、合計 1272 の結果を得た。
...
- `rag_value` と `detail_level` は順序付きの尺度なので、以下の距離スコアを使う:
  ...
  - 2段階ずれなら 1/3
  - 最大ずれなら 0
  式で書くと $1 - |\mathit{pred} - \mathit{gold}| / 3$
...
![Overall Score](image_eval_gold_overall_score.autogen.svg)
...
| `google/gemma-4-26B-A4B-it [thinking=false]` | 0.864 | 86.8% | 0.890 | 0.833 | 43.4% |
| `gpt-5-mini` | 0.864 | 87.7% | 0.928 | 0.783 | 38.7% |
| `gpt-5.4-mini` | 0.864 | 88.7% | 0.921 | 0.777 | 28.3% |
| `Qwen/Qwen3.6-27B-FP8` | 0.862 | 88.7% | 0.921 | 0.770 | 33.0% |
| `gpt-5.4-nano` | 0.848 | 85.8% | 0.915 | 0.767 | 34.9% |
| `Qwen/Qwen3.6-35B-A3B-FP8` | 0.842 | 84.0% | 0.915 | 0.770 | 36.8% |
| `Qwen/Qwen3.6-35B-A3B-FP8 [thinking=false]` | 0.826 | 77.4% | 0.909 | 0.814 | 34.9% |
| `gpt-5-nano` | 0.782 | 82.1% | 0.789 | 0.723 | 27.4% |

### 項目別スコア

![Gold Axis Accuracy](image_eval_gold_axis_accuracy.autogen.svg)
...
| `google/gemma-4-26B-A4B-it [thinking=false]` | 86.8% | 0.330 | 0.890 | 0.500 | 0.833 |
| `gpt-5-mini` | 87.7% | 0.217 | 0.928 | 0.651 | 0.783 |
| `gpt-5.4-mini` | 88.7% | 0.236 | 0.921 | 0.670 | 0.777 |
| `Qwen/Qwen3.6-27B-FP8` | 88.7% | 0.236 | 0.921 | 0.689 | 0.770 |
| `gpt-5.4-nano` | 85.8% | 0.255 | 0.915 | 0.698 | 0.767 |
| `Qwen/Qwen3.6-35B-A3B-FP8` | 84.0% | 0.255 | 0.915 | 0.689 | 0.770 |
| `Qwen/Qwen3.6-35B-A3B-FP8 [thinking=false]` | 77.4% | 0.274 | 0.909 | 0.557 | 0.814 |
| `gpt-5-nano` | 82.1% | 0.632 | 0.789 | 0.830 | 0.723 |

### コストとレイテンシ

![Latency vs Cost](image_eval_latency_cost.autogen.svg)

| モデル条件 | 平均レイテンシ | 中央レイテンシ | 1画像あたり平均コスト | このセット合計コスト |
|---|---:|---:|---:|---:|
| `Qwen/Qwen3.6-27B-FP8` | 175.09s | 143.51s | $0.0000 | $0.0000 |
| `Qwen/Qwen3.6-27B-FP8 [thinking=false]` | 7.47s | 7.43s | $0.0000 | $0.0000 |
| `Qwen/Qwen3.6-35B-A3B-FP8` | 27.02s | 22.51s | $0.0000 | $0.0000 |
| `Qwen/Qwen3.6-35B-A3B-FP8 [thinking=false]` | 1.56s | 1.45s | $0.0000 | $0.0000 |
| `google/gemma-4-26B-A4B-it [thinking=false]` | 1.65s | 1.66s | $0.0000 | $0.0000 |
...
最速は `Qwen/Qwen3.6-35B-A3B-FP8 [thinking=false]` の平均 1.6s だった。スコアは少し落ちてしまう。

OpenAI の古いモデルでは意外と時間がかかる傾向がある。

この screening タスクでは `thinking=true` の採用意義は見えにくい。Qwen の thiking ありなし: `Qwen/Qwen3.6-27B-FP8` の thiking ありとなしでの総合スコアはそれぞれ 0.862, 0.865 と大差なかったにも関わらず、時間差が極端で、平均 175.1s と 7.5s となった。

`google/gemma-4-26B-A4B-it [thinking=false]` は高速でかつ得点も高い。
ただし、gemma モデルには FP8 が用意されていなく、立ち上げ時の online quantization で15分以上待たなければならない。
...
|---|---:|---:|
| `Qwen/Qwen3.6-27B-FP8` | $0.00 | 193.8時間 |
| `Qwen/Qwen3.6-27B-FP8 [thinking=false]` | $0.00 | 8.3時間 |
| `Qwen/Qwen3.6-35B-A3B-FP8` | $0.00 | 29.9時間 |
| `Qwen/Qwen3.6-35B-A3B-FP8 [thinking=false]` | $0.00 | 1.7時間 |
| `google/gemma-4-26B-A4B-it [thinking=false]` | $0.00 | 1.8時間 |
| `gpt-5` | $28.43 | 10.1時間 |
...
総合評価で見ると、第一候補は `gpt-5.4`。

ローカル GPU で速度と品質のバランスが最も良いのは `Qwen/Qwen3.6-27B-FP8 [thinking=false]`。平均 7.5s で、総合スコアは 0.865。 ただし一段目の軽い screening で時間がこれだけかかるということは、後段のテキスト抽出ではさらに時間がかかると思われる。実用に耐えられない場合は `Qwen3.6-35B-A3-FP8` に切り替えるべきと思われる。 `gemma-4-26B-A4B-it` は速度、成績とも良いが、ロード時間が長く気軽にモデル変更できないという欠点がある。
...
| `google/gemma-4-26B-A4B-it [thinking=false]` | `logo_or_mark`→`seal` (4)<br>`text_as_image`→`decorative` (3)<br>`ui_or_screenshot`→`table_or_form` (1)<br>`logo_or_mark`→`decorative` (1)<br>`decorative`→`diagram` (1) |
...
| `Qwen/Qwen3.6-35B-A3B-FP8 [thinking=false]` | `logo_or_mark`→`seal` (4)<br>`decorative`→`other` (2)<br>`decorative`→`diagram` (2)<br>`decorative`→`text_as_image` (2)<br>`diagram`→`table_or_form` (2) |
